...
function App(){
  ...
  const runNode=useCallback(async id=>{const n=nodes.find(x=>x.id===id); const count=await window.htk?.broadcast({type:'run',node:n}); setNotice(count?`Đã gửi tác vụ tới ${count} extension`:'Chưa có extension kết nối');},[nodes]);
